...
from gi.repository import Gtk, Gdk, GObject, GLib, Gst, GdkPixbuf, Gio
import os
import json
import random
import subprocess
...
class MPRISController:
    """Handles MPRIS2 interface for media key controls using Gio."""
    def __init__(self, player):
        self.player = player
        self.bus = Gio.bus_get_sync(Gio.BusType.SESSION, None)
        self.player_path = "/org/mpris/MediaPlayer2"
        self.player_interface = "org.mpris.MediaPlayer2"
        self.player_iface = "org.mpris.MediaPlayer2.Player"

        # Define the MPRIS XML interface
        mpris_xml = """
        ...
        try:
            # Create a Gio.DBusNodeInfo from the XML
            node_info = Gio.DBusNodeInfo.new_for_xml(mpris_xml)

            # Get the interface info (not the node info)
            interface_info = node_info.lookup_interface("org.mpris.MediaPlayer2")
            player_interface_info = node_info.lookup_interface("org.mpris.MediaPlayer2.Player")

            if interface_info and player_interface_info:
                print("✓ MPRIS interfaces loaded successfully")
                # Note: Full D-Bus registration is complex with Gio
                # For now, just initialize properties
                self.registration_id = None
            else:
                print("⚠ MPRIS interface info not found")
                self.registration_id = None

        except Exception as e:
            print(f"MPRIS initialization warning: {e}")
            self.registration_id = None
    ...
    def Position(self):
        try:
            ...
            if success:
                return position / Gst.SECOND * 1000000  # Convert to microseconds
        except:
            pass
        return 0
    ...
    def Seek(self, offset):
        ...
        try:
            position = self.player.playbin.query_position(Gst.Format.TIME)[1]
            new_position = position + offset * 1000  # Convert microseconds to nanoseconds
            self.player.playbin.seek_simple(Gst.Format.TIME, Gst.SeekFlags.FLUSH | Gst.SeekFlags.KEY_UNIT, new_position)
        except Exception as e:
            print(f"MPRIS Seek error: {e}")

    def OpenUri(self, uri):
        # This would open a new track, but our player works with a playlist
        # We'll just ignore this for now or could add the uri to the playlist
        pass

    def update_metadata(self, title, artist, album, uri):
        self._metadata = {
            "mpris:trackid": GLib.Variant("o", "/org/mpris/MediaPlayer2/Track/1"),
            "xesam:title": title,
            "xesam:artist": [artist],
            "xesam:album": album,
            "mpris:artUrl": f"file://{quote(os.path.abspath(uri))}" if not uri.startswith(('http://', 'https://')) else uri,
            "xesam:url": uri
        }
    ...
    def update_playback_status(self, status):
        ...
        self._emit_properties_changed("org.mpris.MediaPlayer2.Player", {"PlaybackStatus": self._playback_status})

    def update_volume(self, volume):
        self._volume = volume
        self._emit_properties_changed("org.mpris.MediaPlayer2.Player", {"Volume": self._volume})

    def _emit_properties_changed(self, interface, properties):
        # This is a simplified version. In a real implementation, you'd use
        # Gio.DBusConnection.emit_signal or similar.
        # For now, we'll just print a message.
        print(f"MPRIS: Emitted PropertiesChanged for {interface}: {properties}")

    def __del__(self):
        # Unregister the object when the controller is destroyed
        if hasattr(self, 'registration_id') and self.registration_id is not None:
            try:
                self.bus.unregister_object(self.registration_id)
                print("MPRIS service unregistered")
            except Exception as e:
                print(f"Failed to unregister MPRIS service: {e}")

class MusicPlayer:
    """
    Carbon Music Player – improved version with:
    - Plasma 6 theming
    - MPRIS2 support for media keys
    - Cleaner, more modern UI inspired by Elisa/Haruna
    ...
    def __init__(self):
        ...
        self._build_header()
        self._build_main()
        # No custom CSS, rely on system theme
        ...
        GLib.timeout_add(100, self._update_progress_bar) # For RGB progress bar animation
    ...
    def _build_window(self):
        ...
        # Use HeaderBar for a more native look
        self.header_bar = Gtk.HeaderBar()
        self.header_bar.set_show_close_button(True)
        self.header_bar.props.title = "Carbon Music Player"
    ...
    def _build_main(self):
        """Kompaktowy layout - wszystko zmieści się na ekranie"""
        ...
        # PLAYLIST + TOOLBAR
        pl_frame = self._frame("Playlist &amp; Controls")
        pl_vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=5)

        # Toolbar z przyciskami (kompaktowy)
        toolbar = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)

        # Przyciski playlist
        add_file = Gtk.Button.new_with_label("+ File")
        add_file.connect("clicked", self._add_file)
        add_dir = Gtk.Button.new_with_label("+ Dir")
        add_dir.connect("clicked", self._add_folder)

        open_btn = Gtk.Button.new_with_label("📂 Open")
        open_btn.set_tooltip_text("Open playlist file")
        open_btn.connect("clicked", self._open_dialog)

        save_pl = Gtk.Button.new_with_label("💾 Save")
        save_pl.connect("clicked", self._save_playlist)

        clear_pl = Gtk.Button.new_with_label("🗑 Clear")
        clear_pl.connect("clicked", lambda b: (self.playlist_store.clear(),
                                                self._update_stats(),
                                                setattr(self, 'current_iter', None)))

        if PYRADIOS_OK:
            radio_btn = Gtk.Button.new_with_label("📻 Radio")
            radio_btn.connect("clicked", self._search_radios)
            toolbar.pack_start(radio_btn, False, False, 0)

        for w in [add_file, add_dir, open_btn, save_pl, clear_pl]:
            toolbar.pack_start(w, False, False, 0)

        # Sort combo
        sort_combo = Gtk.ComboBoxText()
        sort_combo.append_text("Sort...")
        ...
        toolbar.pack_end(self.playlist_stats, False, False, 0)

        pl_vbox.pack_start(toolbar, False, False, 0)
        ...
        pl_vbox.pack_start(self.search_entry, False, False, 0)

        # Playlist view
        self._build_playlist_view()
        sw = Gtk.ScrolledWindow()
        sw.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        sw.set_size_request(500, 300)  # Kompaktowa wysokość
        sw.add(self.playlist_view)
        pl_vbox.pack_start(sw, True, True, 0)

        pl_frame.add(pl_vbox)
        left.pack_start(pl_frame, True, True, 0)

        # CONTROLS + VOLUME + RECENT (w jednym frame)
        ctrl_frame = self._frame("Controls &amp; Settings")
        ctrl_vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=6)

        # Playback controls
        ctrl_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        ...
        shuffle_btn = Gtk.Button.new_with_label("🔀")
        shuffle_btn.connect("clicked", lambda b: self._shuffle())

        for w in [prev_btn, self.btn_play, self.btn_pause, self.btn_stop, next_btn, shuffle_btn]:
            ctrl_hbox.pack_start(w, False, False, 0)
        ctrl_vbox.pack_start(ctrl_hbox, False, False, 0)

        # Repeat mode (kompaktowy)
        repeat_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        off = Gtk.RadioButton.new_with_label_from_widget(None, "Off")
        one = Gtk.RadioButton.new_with_label_from_widget(off, "One")
        allb = Gtk.RadioButton.new_with_label_from_widget(off, "All")
        ...
        repeat_lbl = Gtk.Label(label="Repeat:")
        repeat_hbox.pack_start(repeat_lbl, False, False, 0)
        for w in [off, one, allb]:
            repeat_hbox.pack_start(w, False, False, 0)
        ctrl_vbox.pack_start(repeat_hbox, False, False, 0)

        # Volume (kompaktowy)
        vol_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        ...
        vol_hbox.pack_start(self.mute_btn, False, False, 0)
        vol_hbox.pack_start(vol_lbl, False, False, 0)
        vol_hbox.pack_start(self.gain_scale, True, True, 0)
        ctrl_vbox.pack_start(vol_hbox, False, False, 0)

        # Recent playlists (kompaktowy)
        recent_hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        recent_lbl = Gtk.Label(label="Recent:")
        self.recent_combo = Gtk.ComboBoxText()
        self.recent_combo.append_text("Choose...")
        self.recent_combo.set_active(0)
        self.recent_combo.connect("changed", self._load_recent_from_combo)
        clear_recent = Gtk.Button.new_with_label("Clear")
        clear_recent.connect("clicked", self._clear_recent)
        recent_hbox.pack_start(recent_lbl, False, False, 0)
        recent_hbox.pack_start(self.recent_combo, True, True, 0)
        recent_hbox.pack_start(clear_recent, False, False, 0)
        ctrl_vbox.pack_start(recent_hbox, False, False, 0)

        ctrl_frame.add(ctrl_vbox)
        left.pack_start(ctrl_frame, False, False, 0)

        # NOW PLAYING (kompaktowy)
        now_frame = self._frame("Now Playing")
        now_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=8)

        self.cover_image = Gtk.Image()
        self.cover_image.set_size_request(100, 100)  # Mniejszy cover
        ...
        self.now_title.set_markup("<span weight='bold'>No track</span>")
        self.now_title.set_xalign(0)
        self.now_title.set_line_wrap(True)
        ...
        toggle_remaining = Gtk.Button.new_with_label("⏱")
        toggle_remaining.set_tooltip_text("Toggle remaining time")
        ...
        # VISUALIZATION (z carbon_mp_radio2.py)
        viz_frame = self._frame("Visualization")
        vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4)

        hb_viz = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
        ...
        self.viz_config_btn = Gtk.Button.new_with_label("⚙")
        self.viz_config_btn.set_tooltip_text("Configure visualizer")
        ...
        self.viz_video_box.set_size_request(-1, 200)  # Kompaktowa wysokość
    ...
    def _update_progress_bar(self):
        """Update progress bar animation (called every 100ms)"""
        # Ta metoda może być pusta - _tick_time już aktualizuje progress
        return True

    def _detect_visualizers(self):
        """Wykrywanie wizualizacji z gst-inspect-1.0"""
        visualizers = []

        try:
            # Metoda 1: Użyj gst-inspect-1.0 bez grep
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for line in lines:
                    ...
                    # Szukaj linii zawierających wizualizacje
                    # Format: "plugin:  elementname: Description"
                    if any(keyword in line.lower() for keyword in
                          ['visual', 'scope', 'goom', 'synaes']):
                        parts = line.split(':', 2)
                        if len(parts) >= 2:
                            ...
                            try:
                                ...
                                if test_elem:
                                    ...
                                    print(f"Found visualizer: {element_name} - {description}")
                            except:
                                pass
            else:
                print(f"gst-inspect-1.0 returned code {result.returncode}")

            # Metoda 2: Bezpośrednie sprawdzenie znanych wizualizacji
            known_visualizers = [
                'spacescope', 'wavescope', 'spectrascope', 'synaescope',
                'goom', 'goom2k1', 'monoscope'
            ]

            print("Checking known visualizers...")
            for vis_name in known_visualizers:
                try:
                    elem = Gst.ElementFactory.make(vis_name, None)
                    if elem:
                        # Pobierz długą nazwę z registry
                        factory = Gst.ElementFactory.find(vis_name)
                        if factory:
                            longname = factory.get_longname()
                            if (vis_name, longname) not in visualizers:
                                visualizers.append((vis_name, longname))
                                print(f"✓ Found known visualizer: {vis_name} - {longname}")
                except Exception as e:
                    print(f"  {vis_name}: not available ({e})")
                    pass

            # Metoda 3: Skanowanie registry GStreamer
                    ...
                    for feature in features:
                        try:
                            ...
                            if ("Visualization" in klass or "Visual" in klass or
                                "Scope" in klass or name in known_visualizers):
                                ...
                                if elem:
                                    ...
                                    if (name, longname) not in visualizers:
                                        ...
                                        print(f"Found via registry: {name} - {longname}")
                        except:
                            continue
                except Exception as e:
            ...
            print(f"Visualizer detection error: {e}")
        ...
        if not unique_visualizers:
            print("⚠ No visualization plugins found.")
            print("  Available on your system:")
            print("  $ gst-inspect-1.0 | grep -i 'scope\\|visual\\|goom'")
            print("  Install packages: gstreamer1.0-plugins-good gstreamer1.0-plugins-bad")
        else:
            print(f"✓ Found {len(unique_visualizers)} visualization plugins")
            for name, desc in unique_visualizers:
                print(f"  - {name}: {desc}")

        return unique_visualizers

    def _fill_visualizer_combo(self):
        """Wypełnij combo box wizualizacjami"""
        # Usuń stare handlery
        ...
        active_idx = 0
        if self.selected_visualizer and self.selected_visualizer != "None":
        ...
        self.viz_combo.connect("changed", self._on_visualizer_changed)
        print(f"Visualizer combo filled with {len(self.available_visualizers)} items, active: {active_idx}")
    ...
    def _on_visualizer_changed(self, combo):
        ...
        if text == "None":
            ...
            # Wyciągnij nazwę elementu (przed " — ")
    ...
    def _on_viz_config(self, btn):
        ...
        if not text or text == "None":
            self._notify("No config", "Select a visualizer first")
            return

        vis_name = text.split(" — ", 1)[0].strip()

        if vis_name not in self.vis_properties:
            self._notify("No config", f"No configurable properties for {vis_name}")
            return

        props = self.vis_properties[vis_name]
        config = self.vis_current_config.get(vis_name, {p['name']: p['default'] for p in props})

        dlg = Gtk.Dialog(title=f"Configure {vis_name}", parent=self.window, flags=Gtk.DialogFlags.MODAL)
        ...
        if response == Gtk.ResponseType.OK:
            ...
            self._apply_visualizer(vis_name)
            self._notify("Config saved", f"{vis_name} configuration applied")
        dlg.destroy()

    def _apply_visualizer(self, vis_name: str):
        """Zastosuj wizualizację"""
        self.selected_visualizer = vis_name
        print(f"Applying visualizer: {vis_name}")
        ...
        try:
            ...
            if vis_name == "None":
                try:
                    ...
                    print(f"Clear vis-plugin error: {e}")
            else:
                # Stwórz element wizualizacji
                elem = Gst.ElementFactory.make(vis_name, None)
                if not elem:
    ...
    def _build_header(self):
        ...
        title = Gtk.Label()
        title.set_markup("<span size='large' weight='bold'>🎵 Carbon Music Player</span>")
        title.set_halign(Gtk.Align.START)
        hb.pack_start(title, True, True, 0)

        self.main_box.pack_start(hb, False, False, 0)

    # Dodaj tę metodę do swojej klasy (po _build_gstreamer):
    def _setup_visualization(self):
        """Setup wizualizacji - wywoływane po stworzeniu playbin"""
        # Wykryj dostępne wizualizacje
        print("\n=== Detecting visualizers ===")
        self.available_visualizers = self._detect_visualizers()

        # Wypełnij combo box
        self._fill_visualizer_combo()

        # Podłącz video sink do widgetu (jeśli jeszcze nie jest podłączony)
        try:
            if self.video_sink and hasattr(self.video_sink, 'props') and hasattr(self.video_sink.props, 'widget'):
                widget = self.video_sink.props.widget
                if widget:
                    # Sprawdź czy widget nie jest już dodany
                    if widget.get_parent() is None:
                        self.viz_video_box.pack_start(widget, True, True, 0)
                        self.viz_video_box.show_all()
                        print("✓ Video widget added to GUI")
                    else:
                        print("✓ Video widget already in GUI")
        except Exception as e:
            print(f"Video widget setup error: {e}")

        # Ustaw flagę wizualizacji
        try:
            flags = self.playbin.get_property("flags")
            self.playbin.set_property("flags", int(flags) | 0x0008)  # 0x0008 = GST_PLAY_FLAG_VIS
            print("✓ Visualization flag enabled")
        except Exception as e:
            print(f"Cannot set playbin VIS flag: {e}")

        # Zastosuj wybraną wizualizację
        self._apply_visualizer(self.selected_visualizer)


    def _update_progress_bar(self):
        """Update progress bar with RGB color animation"""
        try:
            ...
            if success and success2 and duration > 0:
                ...
                if self.show_remaining:
                    ...
                    self.progress_bar.set_text(f"{pos_str} / {dur_str} ({rem_str})")
                else:
                    self.progress_bar.set_text(f"{pos_str} / {dur_str}")
            else:
                # No track playing or duration unknown
                self.progress_bar.set_fraction(0.0)
                if not self.progress_bar.get_text() or self.progress_bar.get_text() == "00:00 / 00:00":
                    self.progress_bar.set_text("00:00 / 00:00")
        except Exception as e:
            # Silently handle errors (playbin might not be ready)
            pass
    ...
    def _controls_box(self):
        ...
        shuffle_btn.connect("clicked", lambda b: self._shuffle())
        repeat_btn = Gtk.Button.new_with_label("🔁 Repeat")
        repeat_btn.connect("clicked", lambda b: self._toggle_repeat())

        for w in [self.btn_play, self.btn_pause, self.btn_stop, prev_btn, next_btn, shuffle_btn, repeat_btn]:
    ...
    def _toggle_repeat(self):
        ...
        repeat_btn = self._find_widget_by_text(self._controls_box(), "🔁 Repeat")
        if repeat_btn:
            repeat_btn.set_label(f"🔁 {self.repeat_mode.capitalize()}")

    def _find_widget_by_text(self, container, text):
        # Simple recursive search for a widget with specific text
        for widget in container.get_children():
            if isinstance(widget, Gtk.Button) and widget.get_label() == text:
                ...
                result = self._find_widget_by_text(widget, text)
                if result:
                    return result
        return None
    ...
    def _play(self):
        ...
        self._notify("Now Playing", f"{title}by {artist}")

        # Update MPRIS metadata
        self.mpris.update_metadata(title, artist, album, path)
    ...
    def _stop(self):
        ...
        self.mpris.update_playback_status("Stopped")
    ...
    def _toggle_mute(self):
        ...
        self.mpris.update_volume(self.playbin.get_property("volume"))
    ...
    def _on_bus(self, bus, message):
        try:
            if message.type == Gst.MessageType.EOS:
                ...
                self._error(f"Playback error: {err}{debug}")
                self._next()
    ...
    def _on_destroy(self, w):
        self._save_config()
        self.playbin.set_state(Gst.State.NULL)
        # Unregister MPRIS service
        if hasattr(self, 'mpris') and hasattr(self.mpris, 'registration_id'):
            try:
                self.mpris.bus.unregister_object(self.mpris.registration_id)
                print("MPRIS service unregistered")
            except Exception as e:
                print(f"Failed to unregister MPRIS service: {e}")
        Gtk.main_quit()

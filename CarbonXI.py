...
class MatrixVisualizer(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMinimumHeight(100); self.setMouseTracking(True)
...
# EXTRA FX WIDGET
# ============================================================================
class ExtraFXWidget(QGroupBox):
    """
    Extra FX panel – zaawansowane efekty wpływające na głębię i rozdzielczość audio.
    Działa na obu pipeline'ach: głównym i monitorze.
    """
    PRESETS = {
        "Bypass": {
            "freeverb_room": 0.0, "freeverb_damp": 0.5, "freeverb_level": 0.0,
            "exciter_gain": 0.0,
            "transient_ratio": 1.0, "transient_thresh": 1.0,
            "haas_delay": 1, "haas_intensity": 0.0,
            "oversampling": False,
        },
        "3D Depth Field": {
            "freeverb_room": 0.45, "freeverb_damp": 0.6, "freeverb_level": 0.25,
            "exciter_gain": 0.15,
            "transient_ratio": 1.2, "transient_thresh": 0.85,
            "haas_delay": 18000000, "haas_intensity": 0.3,
            "oversampling": False,
        },
        "Analog Warmth": {
            "freeverb_room": 0.15, "freeverb_damp": 0.8, "freeverb_level": 0.12,
            "exciter_gain": 0.35,
            "transient_ratio": 1.8, "transient_thresh": 0.7,
            "haas_delay": 8000000, "haas_intensity": 0.18,
            "oversampling": False,
        },
        "Crystal Clear": {
            "freeverb_room": 0.08, "freeverb_damp": 0.3, "freeverb_level": 0.08,
            "exciter_gain": 0.25,
            "transient_ratio": 1.05, "transient_thresh": 0.92,
            "haas_delay": 5000000, "haas_intensity": 0.12,
            "oversampling": True,
        },
        "Ultra Resolution": {
            "freeverb_room": 0.22, "freeverb_damp": 0.45, "freeverb_level": 0.18,
            "exciter_gain": 0.2,
            "transient_ratio": 1.1, "transient_thresh": 0.88,
            "haas_delay": 12000000, "haas_intensity": 0.22,
            "oversampling": True,
        },
        "Stadium Wide": {
            "freeverb_room": 0.8, "freeverb_damp": 0.35, "freeverb_level": 0.38,
            "exciter_gain": 0.12,
            "transient_ratio": 1.3, "transient_thresh": 0.75,
            "haas_delay": 30000000, "haas_intensity": 0.45,
            "oversampling": False,
        },
    }

    def __init__(self, parent=None):
        super().__init__("✨ EXTRA FX  —  Depth & Resolution Engine", parent)
        self.setStyleSheet("""
            QGroupBox { color: #FF88FF; border: 1px solid #442244; margin-top: 10px;
                        font-weight: bold; background: #120D14; }
            QLabel { color: #AA88AA; font-size: 10px; }
            QSlider::groove:horizontal { height: 3px; background: #332233; }
            QSlider::handle:horizontal { background: #CC44FF; width: 10px; height: 10px;
                                         margin: -4px 0; border-radius: 5px; }
            QComboBox { background: #1A0D1E; color: #EEE; border: 1px solid #552266;
                        border-radius: 3px; padding: 3px; font-size: 10px; }
            QCheckBox { color: #FF88FF; font-size: 10px; }
            QCheckBox::indicator { width: 12px; height: 12px; border: 1px solid #552266;
                                   border-radius: 2px; background: #120D14; }
            QCheckBox::indicator:checked { background: #CC44FF; }
        """)
        # GStreamer element refs – main pipeline
        self._reverb = None
        self._exciter = None
        self._transient = None
        self._haas = None
        # GStreamer element refs – monitor pipeline
        self._mon_reverb = None
        self._mon_exciter = None
        self._mon_transient = None
        self._mon_haas = None
        # Parent player ref
        self.parent_player = None
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        main.setContentsMargins(8, 16, 8, 8)
        main.setSpacing(5)

        # --- Top row: Enable + Preset select ---
        top = QHBoxLayout()
        ...
        # --- Reverb (Freeverb) ---
        rev_box = QHBoxLayout()
        rev_box.addWidget(QLabel("Reverb Room:"))
        self.reverb_room = self._slider(0, 100, 0)
        self.reverb_room.valueChanged.connect(lambda v: self._apply("reverb_room", v/100.0))
        rev_box.addWidget(self.reverb_room)
        self.reverb_room_lbl = QLabel("0%")
        rev_box.addWidget(self.reverb_room_lbl)

        rev_box.addWidget(QLabel("Level:"))
        self.reverb_level = self._slider(0, 100, 0)
        self.reverb_level.valueChanged.connect(lambda v: self._apply("reverb_level", v/100.0))
        rev_box.addWidget(self.reverb_level)
        self.reverb_level_lbl = QLabel("0%")
        rev_box.addWidget(self.reverb_level_lbl)
        main.addLayout(rev_box)

        # --- Harmonic Exciter (amplification boost) ---
        exc_box = QHBoxLayout()
        exc_box.addWidget(QLabel("Harmonic Exciter:"))
        self.exciter_sl = self._slider(0, 100, 0)
        self.exciter_sl.valueChanged.connect(lambda v: self._apply("exciter_gain", v/100.0))
        exc_box.addWidget(self.exciter_sl)
        self.exciter_lbl = QLabel("0%")
        exc_box.addWidget(self.exciter_lbl)
        main.addLayout(exc_box)

        # --- Transient Shaper (Comp ratio) ---
        trans_box = QHBoxLayout()
        trans_box.addWidget(QLabel("Transient Shaper:"))
        self.transient_sl = self._slider(0, 100, 0)
        self.transient_sl.valueChanged.connect(lambda v: self._apply("transient_ratio", 1.0 + v/50.0))
        trans_box.addWidget(self.transient_sl)
        self.transient_lbl = QLabel("1.0:1")
        trans_box.addWidget(self.transient_lbl)
        main.addLayout(trans_box)

        # --- Haas Stereo Delay ---
        haas_box = QHBoxLayout()
        haas_box.addWidget(QLabel("Haas Depth:"))
        self.haas_sl = self._slider(0, 100, 0)
        self.haas_sl.valueChanged.connect(lambda v: self._apply("haas_delay", v * 400000 + 1))
        haas_box.addWidget(self.haas_sl)
        self.haas_lbl = QLabel("0ms")
        haas_box.addWidget(self.haas_lbl)
        main.addLayout(haas_box)

        # --- Oversampling toggle ---
        over_box = QHBoxLayout()
        self.oversample_cb = QCheckBox("⬆ Oversampling / Anti-alias (audioresample quality boost)")
        self.oversample_cb.toggled.connect(lambda v: self._apply("oversampling", v))
        over_box.addWidget(self.oversample_cb)
        main.addLayout(over_box)

        info = QLabel("💡 Reverb=przestrzeń  Exciter=harmoniczne  Transient=dynamika  Haas=szerokość")
        info.setStyleSheet("color: #664466; font-size: 9px; font-style: italic;")
        main.addWidget(info)
    ...
    def set_pipeline(self, reverb, exciter, transient, haas,
                     mon_reverb=None, mon_exciter=None, mon_transient=None, mon_haas=None):
        """Connect to GStreamer elements from main and monitor pipelines."""
        self._reverb = reverb
        self._exciter = exciter
        self._transient = transient
        self._haas = haas
        self._mon_reverb = mon_reverb
        self._mon_exciter = mon_exciter
        self._mon_transient = mon_transient
        self._mon_haas = mon_haas
        if self.enable_cb.isChecked():
            self._on_enable(True)

    def _on_enable(self, enabled):
        if not enabled:
            # Bypass all – apply zero/flat values directly
            self._set_reverb(None, 0.0, 0.0, 0.5)
            self._set_exciter(None, 1.0)
            self._set_transient(None, 1.0, 1.0)
            self._set_haas(None, 1, 0.0)
        else:
            # Re-apply current slider values
            self._apply("reverb_room", self.reverb_room.value() / 100.0)
            self._apply("reverb_level", self.reverb_level.value() / 100.0)
            self._apply("exciter_gain", self.exciter_sl.value() / 100.0)
            self._apply("transient_ratio", 1.0 + self.transient_sl.value() / 50.0)
            self._apply("haas_delay", self.haas_sl.value() * 400000 + 1)

    def _apply(self, param, value):
        if not self.enable_cb.isChecked():
            return
        if param == "reverb_room":
            self.reverb_room_lbl.setText(f"{int(value*100)}%")
            self._set_reverb(param, value, None, None)
        elif param == "reverb_level":
            self.reverb_level_lbl.setText(f"{int(value*100)}%")
            self._set_reverb(param, None, value, None)
        elif param == "exciter_gain":
            self.exciter_lbl.setText(f"{int(value*100)}%")
            # amplification: 1.0 = unity, +gain adds brightness
            amp = 1.0 + value * 0.8
            self._set_exciter(param, amp)
        elif param == "transient_ratio":
            self.transient_lbl.setText(f"{value:.2f}:1")
            thresh = max(0.05, 1.0 - (value - 1.0) * 0.4)
            self._set_transient(param, value, thresh)
        elif param == "haas_delay":
            ms = int(value / 1000000)
            self.haas_lbl.setText(f"{ms}ms")
            intensity = 0.0 if value <= 1 else 0.4
            self._set_haas(param, int(value), intensity)
        elif param == "oversampling":
            # Set audioresample quality: 0=fast, 10=sinc
            quality = 10 if value else 4
            self._set_oversampling(quality)

    def _set_reverb(self, param, room, level, damp):
        for el in [self._reverb, self._mon_reverb]:
            if not el: continue
            try:
                if room is not None:
                    el.set_property("room-size", float(room))
                if level is not None:
                    el.set_property("level", float(level))
                if damp is not None:
                    el.set_property("damping", float(damp))
            except Exception as e:
                print(f"ExtraFX reverb error: {e}")

    def _set_exciter(self, param, amp):
        for el in [self._exciter, self._mon_exciter]:
            if not el: continue
            try:
                el.set_property("amplification", float(amp))
            except Exception as e:
                print(f"ExtraFX exciter error: {e}")

    def _set_transient(self, param, ratio, thresh):
        for el in [self._transient, self._mon_transient]:
            if not el: continue
            try:
                el.set_property("ratio", float(ratio))
                el.set_property("threshold", float(thresh))
            except Exception as e:
                print(f"ExtraFX transient error: {e}")

    def _set_haas(self, param, delay_ns, intensity):
        for el in [self._haas, self._mon_haas]:
            if not el: continue
            try:
                el.set_property("delay", max(1, int(delay_ns)))
                el.set_property("intensity", float(intensity))
            except Exception as e:
                print(f"ExtraFX haas error: {e}")

    def _set_oversampling(self, quality):
        """Try to set audioresample quality on parent pipeline resamplers."""
        if not self.parent_player:
            return
        for attr in ['_main_resample', '_mon_resample']:
            el = getattr(self.parent_player, attr, None)
            if el:
                try:
                    el.set_property("quality", quality)
                except:
                    pass
    ...
    def _load_preset(self, name):
        ...
        # Enable/disable checkbox silently
        was_blocked = self.enable_cb.blockSignals(True)
        self.enable_cb.setChecked(not bypass)
        self.enable_cb.blockSignals(was_blocked)

        # Set sliders
        self.reverb_room.setValue(int(p["freeverb_room"] * 100))
        self.reverb_level.setValue(int(p["freeverb_level"] * 100))
        self.exciter_sl.setValue(int(p["exciter_gain"] * 100))
        self.transient_sl.setValue(int((p["transient_ratio"] - 1.0) * 50))
        haas_ms = max(0, int((p["haas_delay"] - 1) / 400000))
        self.haas_sl.setValue(min(100, haas_ms))
        self.oversample_cb.setChecked(p.get("oversampling", False))
...
class CarbonPhaserPlayer(QMainWindow):
    ...
    def setup(self):
        ...
        # --- EXTRA FX (Depth & Resolution Engine) ---
        self.extra_fx = ExtraFXWidget()
        self.extra_fx.parent_player = self
        rl.addWidget(self.extra_fx)
    ...
    def gst_init(self):
        ...
        # ---- EXTRA FX elements ----
        # 1. Freeverb (room reverb / 3D depth)
        self.extra_reverb = Gst.ElementFactory.make("freeverb", "extra_reverb")
        if self.extra_reverb:
            self.extra_reverb.set_property("room-size", 0.0)
            self.extra_reverb.set_property("level", 0.0)
            self.extra_reverb.set_property("damping", 0.5)

        # 2. Harmonic Exciter (audioamplify for brightness/warmth)
        self.extra_exciter = Gst.ElementFactory.make("audioamplify", "extra_exciter")
        if self.extra_exciter:
            self.extra_exciter.set_property("amplification", 1.0)

        # 3. Transient Shaper (audiodynamic – separate from spatial_sat)
        self.extra_transient = Gst.ElementFactory.make("audiodynamic", "extra_transient")
        if self.extra_transient:
            self.extra_transient.set_property("characteristics", "soft-knee")
            self.extra_transient.set_property("mode", "compressor")
            self.extra_transient.set_property("ratio", 1.0)
            self.extra_transient.set_property("threshold", 1.0)

        # 4. Extra Haas echo (independent depth layer)
        self.extra_haas = Gst.ElementFactory.make("audioecho", "extra_haas")
        if self.extra_haas:
            self.extra_haas.set_property("delay", 1)
            self.extra_haas.set_property("intensity", 0.0)
            self.extra_haas.set_property("feedback", 0.0)

        # Audioresample ref for oversampling quality control
        self._main_resample = Gst.ElementFactory.make("audioresample", "main_resample")
        ...
        if self.eq:
            ...
            # Extra FX elements
            if self._main_resample:
                self.pipeline_bin.add(self._main_resample)
            if self.extra_reverb:
                self.pipeline_bin.add(self.extra_reverb)
            if self.extra_exciter:
                self.pipeline_bin.add(self.extra_exciter)
            if self.extra_transient:
                self.pipeline_bin.add(self.extra_transient)
            if self.extra_haas:
                self.pipeline_bin.add(self.extra_haas)

            self.pipeline_bin.add(self.sp)
            self.pipeline_bin.add(sink)

            # Link chain: conv -> resample -> tape_sat -> tape_gain -> tape_tone -> EQ
            conv.link(self._main_resample if self._main_resample else tape_sat)
            if self._main_resample:
                self._main_resample.link(tape_sat)
            ...
            # Extra FX chain (depth & resolution)
            if self.extra_reverb:
                last_element.link(self.extra_reverb)
                last_element = self.extra_reverb
            if self.extra_exciter:
                last_element.link(self.extra_exciter)
                last_element = self.extra_exciter
            if self.extra_transient:
                last_element.link(self.extra_transient)
                last_element = self.extra_transient
            if self.extra_haas:
                last_element.link(self.extra_haas)
                last_element = self.extra_haas
        ...
        # Connect Extra FX widget to pipeline elements
        if hasattr(self, 'extra_fx'):
            self.extra_fx.set_pipeline(
                self.extra_reverb if hasattr(self, 'extra_reverb') else None,
                self.extra_exciter if hasattr(self, 'extra_exciter') else None,
                self.extra_transient if hasattr(self, 'extra_transient') else None,
                self.extra_haas if hasattr(self, 'extra_haas') else None,
            )
    ...
    def pl_t(self,i): 
        ...
        if uri.startswith("pulsesrc://"):
            ...
            # Extra FX for monitor pipeline
            mon_extra_reverb = Gst.ElementFactory.make("freeverb", "mon-extra-reverb")
            if mon_extra_reverb:
                # Copy values from main extra_reverb
                if hasattr(self, 'extra_reverb') and self.extra_reverb:
                    mon_extra_reverb.set_property("room-size", self.extra_reverb.get_property("room-size"))
                    mon_extra_reverb.set_property("level", self.extra_reverb.get_property("level"))
                    mon_extra_reverb.set_property("damping", self.extra_reverb.get_property("damping"))
                else:
                    mon_extra_reverb.set_property("room-size", 0.0)
                    mon_extra_reverb.set_property("level", 0.0)
                    mon_extra_reverb.set_property("damping", 0.5)

            mon_extra_exciter = Gst.ElementFactory.make("audioamplify", "mon-extra-exciter")
            if mon_extra_exciter:
                amp = self.extra_exciter.get_property("amplification") if hasattr(self, 'extra_exciter') and self.extra_exciter else 1.0
                mon_extra_exciter.set_property("amplification", amp)

            mon_extra_transient = Gst.ElementFactory.make("audiodynamic", "mon-extra-transient")
            if mon_extra_transient:
                mon_extra_transient.set_property("characteristics", "soft-knee")
                mon_extra_transient.set_property("mode", "compressor")
                if hasattr(self, 'extra_transient') and self.extra_transient:
                    mon_extra_transient.set_property("ratio", self.extra_transient.get_property("ratio"))
                    mon_extra_transient.set_property("threshold", self.extra_transient.get_property("threshold"))
                else:
                    mon_extra_transient.set_property("ratio", 1.0)
                    mon_extra_transient.set_property("threshold", 1.0)

            mon_extra_haas = Gst.ElementFactory.make("audioecho", "mon-extra-haas")
            if mon_extra_haas:
                if hasattr(self, 'extra_haas') and self.extra_haas:
                    mon_extra_haas.set_property("delay", max(1, self.extra_haas.get_property("delay")))
                    mon_extra_haas.set_property("intensity", self.extra_haas.get_property("intensity"))
                    mon_extra_haas.set_property("feedback", 0.0)
                else:
                    mon_extra_haas.set_property("delay", 1)
                    mon_extra_haas.set_property("intensity", 0.0)
                    mon_extra_haas.set_property("feedback", 0.0)

            self._mon_extra_reverb = mon_extra_reverb
            self._mon_extra_exciter = mon_extra_exciter
            self._mon_extra_transient = mon_extra_transient
            self._mon_extra_haas = mon_extra_haas
            self._mon_resample = audioresample
            ...
            # Add Extra FX elements to monitor pipeline
            if mon_extra_reverb: self._monitor_pipeline.add(mon_extra_reverb)
            if mon_extra_exciter: self._monitor_pipeline.add(mon_extra_exciter)
            if mon_extra_transient: self._monitor_pipeline.add(mon_extra_transient)
            if mon_extra_haas: self._monitor_pipeline.add(mon_extra_haas)
            ...
            # Link Extra FX chain before spectrum
            mon_last = audioconvert_spatial3
            if mon_extra_reverb:
                mon_last.link(mon_extra_reverb); mon_last = mon_extra_reverb
            if mon_extra_exciter:
                mon_last.link(mon_extra_exciter); mon_last = mon_extra_exciter
            if mon_extra_transient:
                mon_last.link(mon_extra_transient); mon_last = mon_extra_transient
            if mon_extra_haas:
                mon_last.link(mon_extra_haas); mon_last = mon_extra_haas

            if not mon_last.link(spectrum):
                print("❌ Failed to link extra FX to spectrum")
                return
            ...
            if hasattr(self, 'extra_fx'):
                self.extra_fx.set_pipeline(
                    self.extra_reverb, self.extra_exciter, self.extra_transient, self.extra_haas,
                    mon_extra_reverb, mon_extra_exciter, mon_extra_transient, mon_extra_haas
                )
        ...
        # Check if it's a TV video stream (explicitly tagged)
        is_tv = "[TV]" in self.pl[i][1]
        ...
        if is_tv:
            ...
            # Also play audio through GStreamer (for spectrum/FX)
            print(f"🎬 Playing TV stream: {uri}")
            self.ply.set_state(Gst.State.NULL)
            self.ply.get_state(Gst.CLOCK_TIME_NONE)
            self.ply.set_property("audio-sink", self.pipeline_bin)
            ...
            # Use audio player and visualizer (files AND radio streams)
            self.display_stack.setCurrentIndex(0)  # Switch to visualizer
            self.video_player.stop()
            
            print(f"🎵 Playing {'radio' if '[Radio]' in self.pl[i][1] else 'audio'}: {uri}")
            ...
            # CRITICAL: Always re-set audio-sink before playing
            # (it can be lost if pipeline was in NULL state or after monitor mode)
            self.ply.set_property("audio-sink", self.pipeline_bin)
            audio_sink = self.ply.get_property("audio-sink")
            print(f"  Audio sink set: {audio_sink}")

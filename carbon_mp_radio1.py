...
class MusicPlayer:
    ...
    def _build_equalizer_ui(self):
        ...
        # Combo box dla kierunku wygładzania
    ...
    def _autofit_eq_from_spectrum(self, btn=None):
        if self.autofit_active:
            ...
            if hasattr(self, 'spectrum') and self.spectrum:
                try:
                    magnitudes = getattr(self, 'last_magnitudes', [])
                    if magnitudes:
    ...
    def _process_spectrum_message(self, message):
        ...
        try:
            ...
            self.last_magnitudes = magnitudes_list
            ...
            smoothing_curves = {
                "Rising": [-6, -4, -2, 0, 2, 4, 6, 8, 10, 12][:num_bands],
                "Falling": [12, 10, 8, 6, 4, 2, 0, -2, -4, -6][:num_bands],
                "Half Rising": [0, 0, 0, 0, 0, 2, 4, 6, 8, 10][:num_bands],
                "Half Falling": [10, 8, 6, 4, 2, 0, 0, 0, 0, 0][:num_bands],
                "Inverse Rising": [12, 10, 8, 6, 4, 2, 0, -2, -4, -6][:num_bands],
                "Inverse Falling": [-6, -4, -2, 0, 2, 4, 6, 8, 10, 12][:num_bands],
                "Inverse Half Rising": [10, 8, 6, 4, 2, 0, 0, 0, 0, 0][:num_bands],
                "Inverse Half Falling": [0, 0, 0, 0, 0, 2, 4, 6, 8, 10][:num_bands],
            }
            ...
            for i in range(num_bands):
                try:
                    ...
                    if mag_db < -50:
                        eq_adjustment = 2.0 + ((-50 - mag_db) / 30.0) * 2.0
                    elif mag_db < -20:
                        eq_adjustment = (mag_db + 35) / 15.0
                    else:
                        eq_adjustment = -1.0 + ((mag_db + 20) / 20.0) * (-2.0)

                    if smooth_type != "None" and smooth_type in smoothing_curves:
                        smooth_adjust = smoothing_curves[smooth_type][i]
                        eq_adjustment = (eq_adjustment * 0.5 + smooth_adjust * 0.5)
            ...
            if self._spectrum_error_count <= 2:

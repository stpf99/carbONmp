...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QButtonGroup, QInputDialog, QMessageBox, QDial) # Added QDial
        ...
        if ok and tag:
            try:
                ...
                if not stations:
                    ...
                    return
                for s in stations:
                    name = s.get('name', 'Unknown').strip(); url = s.get('url_resolved')
                    if url and name:
                        display_name = f"[Radio] {name}"
                        self.pl.append((url, display_name)); self.ls.addItem(display_name)
                if not self.play and self.idx == -1 and self.pl: self.pl_t(0)
            except Exception as e: QMessageBox.critical(self, "Error", f"Radio Error: {e}")

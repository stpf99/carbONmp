...
class MusicPlayer:
    ...
    def __init__(self):
        ...
        GLib.timeout_add(500, self._update_playlist_highlight)  # For blinking effect
    ...
    def _update_playlist_highlight(self):
        ...
        # Force redraw of playlist
        if hasattr(self, 'playlist_view') and self.current_iter:
    ...
    def _rebuild_eq_sliders(self):
        ...
        for i in range(bands):
            ...
            # Removed set_inverted(True) to have + at top, - at bottom
    ...
    # Don't forget to add this to __init__ after vis_properties:
    # self.vis_current_config = {}
    ...
    def _maybe_update_cover_from_tags(self, taglist: Gst.TagList, current_uri: str = None):
        ...
        # Try remote ID3 for streams
        if current_uri and current_uri.lower().startswith(("http://", "https://")) and EYE3D_OK:
            try:
                import io
                req = Request(current_uri, headers={"User-Agent": "Mozilla/5.0 (Carbon Player)"})
                with urlopen(req, timeout=5) as resp:
                    data = resp.read()

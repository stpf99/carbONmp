...
def get_metadata(uri, fn):
    ...
    # Obsługa Radia (Parsowanie nazwy z playlisty)
    if "[Radio]" in fn:
        a = "Internet Radio"
        t = fn.replace("[Radio] ", "")
...
class CarbonPhaserPlayer(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Carbon Player v13 (Radio Active)")
        self.resize(1200, 950); self.setStyleSheet("QMainWindow{background:#121214}QWidget{color:#DDD;font-family:'Segoe UI'}QListWidget{background:#18181C;border:none}QListWidget::item:selected{background:#0088CC}QPushButton{background:#2A2A30;border:1px solid #3A3A40;padding:5px;border-radius:4px}QPushButton:hover{border-color:#00AAAA}QComboBox{background:#2A2A30;color:#EEE;border:1px solid #333}")
        self.pl=[]; self.idx=-1; self.play=False; Gst.init(None); self.setup(); self.gst_init()
    ...
    def setup(self):
        ...
        # --- PRZYCISK RADIO DODANY TUTAJ ---
        bh=QHBoxLayout()
        ba=QPushButton("Add"); ba.clicked.connect(self.add)
        br=QPushButton("Radio"); br.clicked.connect(self.search_radio) # <--- RADIO
        bc=QPushButton("Clear"); bc.clicked.connect(self.clr)
        bh.addWidget(ba); bh.addWidget(br); bh.addWidget(bc); ll.addLayout(bh); m.addWidget(lp)
        # -----------------------------------
        ...
        ph.addWidget(QLabel("PRESET:")); cb=QComboBox(); self.viz=MatrixVisualizer(); cb.addItems(list(self.viz.presets.keys())); cb.currentTextChanged.connect(self.viz.set_preset); ph.addWidget(cb); ph.addStretch(); vl.addLayout(ph); vl.addWidget(self.viz,1); rl.addWidget(vc,1)

        self.eqw=EqualizerWidget(); self.phaser = PhaserWidget(self.viz, self.eqw); rl.addWidget(self.phaser); rl.addWidget(self.eqw)
    ...
    def gst_init(self):
        self.ply=Gst.ElementFactory.make("playbin","p"); b=Gst.Bin.new("a"); c=Gst.ElementFactory.make("audioconvert","c"); self.eq=Gst.ElementFactory.make("equalizer-10bands","e"); self.sp=Gst.ElementFactory.make("spectrum","s"); self.sp.set_property("bands",64); self.sp.set_property("threshold",-80); self.sp.set_property("post-messages",True); self.sp.set_property("message-magnitude",True); k=Gst.ElementFactory.make("autoaudiosink","k")
        if self.eq: self.eqw.set_gst(self.eq); b.add(c); b.add(self.eq); b.add(self.sp); b.add(k); c.link(self.eq); self.eq.link(self.sp); self.sp.link(k)
        else: self.eqw.setEnabled(False); b.add(c); b.add(self.sp); b.add(k); c.link(self.sp); self.sp.link(k)
        b.add_pad(Gst.GhostPad.new("sink",c.get_static_pad("sink"))); self.ply.set_property("audio-sink",b); self.bus=self.ply.get_bus()
    ...
    def search_radio(self):
        ...
        tag, ok = QInputDialog.getText(self, "Internet Radio", "Enter Genre/Tag (e.g. ambient, news, jazz):")
        if ok and tag:
            try:
                rb = RadioBrowser()
                stations = rb.search(tag=tag, limit=15)
                if not stations:
                    QMessageBox.information(self, "Info", "No stations found.")
                    return
                ...
                for s in stations:
                    ...
                    if url and name:
                        # Oznaczamy jako radio w nazwie
                        display_name = f"[Radio] {name}"
                        self.pl.append((url, display_name))
                        self.ls.addItem(display_name)

                # Jeśli nic nie grało, zacznij grać pierwszą znalezioną
                if not self.play and self.idx == -1 and self.pl:
                    self.pl_t(0)

            except Exception as e:
                QMessageBox.critical(self, "Error", f"Radio Error: {e}")
    # ---------------------------
    ...
    def pl_t(self,i): self.idx=i; self.ply.set_state(Gst.State.NULL); self.ply.set_property("uri",self.pl[i][0]); self.ply.set_state(Gst.State.PLAYING); self.play=True; self.bp.setText("Pause"); self.lt.setText(self.pl[i][1]); self.ls.setCurrentRow(i); self.up_m()
    def pp(self):
        if not self.pl: return
        if self.idx==-1: self.pl_t(0); return
        if self.play: self.ply.set_state(Gst.State.PAUSED); self.play=False; self.bp.setText("Play")
        else: self.ply.set_state(Gst.State.PLAYING); self.play=True; self.bp.setText("Pause")
    ...
    def add(self):
        f,_=QFileDialog.getOpenFileNames(self,"Add","","Audio (*.mp3 *.flac *.m3u)")
        for p in f: self.pl.append(("file:///"+p.replace("\\","/"),os.path.basename(p))); self.ls.addItem(os.path.basename(p))
        if not self.play and self.idx==-1 and self.pl: self.pl_t(0)
    def clr(self): self.ply.set_state(Gst.State.NULL); self.play=False; self.pl=[]; self.ls.clear(); self.idx=-1; self.up_m()

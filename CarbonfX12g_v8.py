...
class PhantomStereoWidget(QGroupBox):
    ...
    Posiada WŁASNE, niezależne elementy GStreamer (ph_conv1, ph_stereo, ph_conv2, ph_echo)
    wstawione ZA blokiem Spatial FX. Bypass przez neutralne wartości właściwości.
    Elementy są rejestrowane w pipeline przez set_pipeline() i nigdy nie kolidują z Spatial.

    Topologia w pipeline:
        ... sp_echo → ph_conv1 → ph_stereo → ph_conv2 → ph_echo → conv_out ...

    Bypass (ENABLE off):
        ph_stereo.stereo = 1.0  (brak poszerzenia)
        ph_echo.intensity = 0.0 (brak echa)
    """
    SS = (
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(self.SS)
        # GST elements — WŁASNE, tworzone przez create_gst_elements()
        # main pipeline
        self.ph_conv1  = None
        self.ph_stereo = None
        self.ph_conv2  = None
    ...
    def _build(self):
        ...
        self._sl_width   = mk_row("Stereo Width:",   "width",   0,  100,  20, "%")
        self._sl_delay   = mk_row("Haas Delay:",      "delay",   0,   20,   8, " ms")
        self._sl_blend   = mk_row("Crossfeed Blend:", "blend",   0,   30,   5, "%")
    ...
    # ── GStreamer — tworzenie własnych elementów ───────────────────────────────

    def create_gst_elements(self, pipeline, prefix="ph"):
        """
        Tworzy własne elementy GST i dodaje je do pipeline.
        Zwraca (conv1, stereo_el, conv2, echo_el) — gotowe do linkowania.
        Elementy startują w trybie bypass (stereo=1.0, echo intensity=0.0).
        """
        c1  = mkgst("audioconvert", f"{prefix}_conv1")
        sw  = mkgst("stereo",       f"{prefix}_stereo", {"stereo": 1.0})
        c2  = mkgst("audioconvert", f"{prefix}_conv2")
        eco = mkgst("audioecho",    f"{prefix}_echo",
                    {"delay": 1, "intensity": 0.0, "feedback": 0.0})
        for el in [c1, sw, c2, eco]:
            if el: pipeline.add(el)
        # Link wewnętrzny bloku
        def lnk(a, b):
            if a and b:
                if not a.link(b):
                    print(f"  [!] phantom link: {a.get_name()}->{b.get_name()}")
        lnk(c1, sw); lnk(sw, c2); lnk(c2, eco)
        return c1, sw, c2, eco

    def set_pipeline(self, pipeline, prev_el, next_el,
                     mon_pipeline=None, mon_prev_el=None, mon_next_el=None):
        """
        Tworzy własne elementy GST, wstawia je między prev_el a next_el.
        prev_el → ph_conv1 → ph_stereo → ph_conv2 → ph_echo → next_el

        Dla monitor pipeline (opcjonalne): analogicznie z mph_*.
        UWAGA: należy wywołać przed set_state(PLAYING) lub po eject/inject limiterów.
        """
        self.ph_conv1, self.ph_stereo, self.ph_conv2, self.ph_echo = \
            self.create_gst_elements(pipeline, "ph")
        ...
        # Odpinamy istniejące połączenie prev_el → next_el
        if prev_el and next_el:
            prev_el.unlink(next_el)
        # Wpinamy własny blok
        lnk(prev_el,       self.ph_conv1)
        lnk(self.ph_echo,  next_el)

        # Monitor pipeline
        if mon_pipeline and mon_prev_el and mon_next_el:
            self.mph_conv1, self.mph_stereo, self.mph_conv2, self.mph_echo = \
                self.create_gst_elements(mon_pipeline, "mph")
            if mon_prev_el and mon_next_el:
                mon_prev_el.unlink(mon_next_el)
            lnk(mon_prev_el,       self.mph_conv1)
            lnk(self.mph_echo,     mon_next_el)

        # Zastosuj aktualny stan (bypass jeśli ENABLE off)
        self._apply_current()
    ...
    def _s(self, el, prop, val):
        if el:
            try: el.set_property(prop, val)
                ...
                pass  # ciche — różnice między pluginami
    ...
    def _bypass(self):
        """Przywraca neutralne wartości — sygnał przechodzi bez zmian."""
        for sw in [self.ph_stereo, self.mph_stereo]:
            self._s(sw, "stereo", 1.0)
        for eco in [self.ph_echo, self.mph_echo]:
            self._s(eco, "delay",     1)
            self._s(eco, "intensity", 0.0)
            self._s(eco, "feedback",  0.0)
    ...
    def _apply_current(self):
        ...
        pairs = [(self.ph_stereo, self.ph_echo), (self.mph_stereo, self.mph_echo)]

        if mode == "haas":
            delay_ns = max(1, delay * 1_000_000)   # ms → ns
            intensity = 0.25 if delay > 0 else 0.0
            for sw, eco in pairs:
                self._s(sw,  "stereo",    max(0.0, min(1.0, width)))
                self._s(eco, "delay",     delay_ns)
                self._s(eco, "intensity", intensity)
                self._s(eco, "feedback",  0.0)
            self._mode_lbl.setText(f"● HAAS  {delay}ms  width={width:.0%}")

        elif mode == "crossfeed":
            intensity = max(0.0, min(0.4, blend))
            for sw, eco in pairs:
                self._s(sw,  "stereo",    max(0.0, min(1.0, width)))
                self._s(eco, "delay",     1_000_000)   # 1ms stałe
                self._s(eco, "intensity", intensity)
                self._s(eco, "feedback",  0.0)
...
# ============================================================================
class TestSignalGenerator(QGroupBox):
    """
    Wbudowany generator sygnału testowego — osobny pipeline GStreamer.
    audiotestsrc → vol → audioconvert → audioresample → autoaudiosink

    Fale: sine, sawtooth, triangle, square, white-noise + auto-remix (kolejność cykliczna).
    Sweep: auto-sweep (center→low→high→center) + ręczny suwak.
    Tempo: wolne / normalne / szybkie (dla zarówno sweep jak i morfowania fal).
    """
    WAVES = [
        ("Sinus",    0),
        ("Piła",     1),
    ...
    TEMPO_MS   = {"🐢 Wolno": 80,   "▶ Normal": 30,   "⚡ Szybko": 10}
    TEMPO_STEP = {"🐢 Wolno": 1.008, "▶ Normal": 1.025, "⚡ Szybko": 1.06}
    WAVE_HOLD  = {"🐢 Wolno": 3000,  "▶ Normal": 1500,  "⚡ Szybko": 500}

    SS = (
        "QGroupBox{color:#00DDFF;border:1px solid #004466;"
        "margin-top:10px;font-weight:bold;background:#060E12}"
        "QLabel{color:#336677;font-size:9px}"
        "QSlider::groove:horizontal{height:3px;background:#0A1A22;border-radius:1px}"
        "QSlider::handle:horizontal{background:#00DDFF;width:10px;height:10px;"
        "margin:-3px 0;border-radius:5px}"
        "QCheckBox{color:#00DDFF;font-weight:bold;spacing:4px}"
        "QCheckBox::indicator{width:12px;height:12px;border:1px solid #00AACC;"
        "border-radius:2px;background:#060E12}"
        "QCheckBox::indicator:checked{background:#00AACC}"
        "QPushButton{background:#0A1A22;color:#00DDFF;border:1px solid #004466;"
        "padding:3px 10px;border-radius:4px;font-size:10px;font-weight:bold}"
        "QPushButton:checked{background:#004466;border-color:#00DDFF;color:#FFFFFF}"
        "QPushButton:hover{border-color:#00DDFF}"
        "QComboBox{background:#0A1A22;color:#00DDFF;border:1px solid #004466;"
        "border-radius:3px;padding:3px 6px;font-size:10px}"
        "QComboBox QAbstractItemView{background:#0A1A22;color:#00DDFF;"
        "selection-background-color:#004466}"
    )

    def __init__(self, parent=None):
        super().__init__("🔬 Generator Sygnału Testowego", parent)
        self.setStyleSheet(self.SS)
        self._pipe = None
        self._src  = None
        self._vol_el = None
        self._active = False
        self._app  = None   # referencja do CarbonPhaserPlayer (set via set_app())
        self._sweep_freq  = float(self.FREQ_CENTER)
        self._sweep_phase = 0
        self._remix_idx   = 0
        self._sweep_timer = QTimer(); self._sweep_timer.timeout.connect(self._sweep_tick)
        self._remix_timer = QTimer(); self._remix_timer.timeout.connect(self._remix_tick)
        self._build()
    ...
    def _lbl_sec(self, t):
        l = QLabel(t)
        l.setStyleSheet("color:#00AACC;font-size:9px;font-weight:bold;letter-spacing:1px")
        return l

    def _sep(self):
        s = QFrame(); s.setFrameShape(QFrame.Shape.HLine)
        s.setStyleSheet("QFrame{color:#003344}"); return s

    def _build(self):
        root = QVBoxLayout(self)
        root.setContentsMargins(8,16,8,8); root.setSpacing(6)

        # Row 1: START + Vol
        r1 = QHBoxLayout(); r1.setSpacing(8)
        self.btn_start = QPushButton("▶ START GENERATOR")
        self.btn_start.setCheckable(True); self.btn_start.setFixedHeight(28)
        self.btn_start.toggled.connect(self._on_start)
        r1.addWidget(self.btn_start, 2)
        r1.addWidget(QLabel("Vol:"))
        self.vol_sl = QSlider(Qt.Orientation.Horizontal)
        self.vol_sl.setRange(0,100); self.vol_sl.setValue(60); self.vol_sl.setFixedWidth(80)
        self.vol_sl.valueChanged.connect(self._on_vol)
        self.vol_lbl = QLabel("60%"); self.vol_lbl.setFixedWidth(32)
        self.vol_sl.valueChanged.connect(lambda v: self.vol_lbl.setText(f"{v}%"))
        r1.addWidget(self.vol_sl); r1.addWidget(self.vol_lbl)
        root.addLayout(r1)

        # Info — jak działa routing
        info = QLabel(
            "💡 Generator wysyła sygnał przez cały łańcuch FX (Tape → Spatial → "
            "Phantom → EQ → Signal Chain → Limitery). Kliknij START — Monitor Mode "
            "uruchomi się automatycznie."
        )
        info.setWordWrap(True)
        info.setStyleSheet(
            "color:#226644;font-size:9px;background:#060E12;"
            "border:1px solid #003322;border-radius:3px;padding:4px 6px")
        root.addWidget(info)
        r2 = QHBoxLayout(); r2.setSpacing(6)
        r2.addWidget(QLabel("Tempo:"))
        self.tempo_cb = QComboBox()
        self.tempo_cb.addItems(list(self.TEMPO_MS.keys()))
        self.tempo_cb.setCurrentIndex(1)
        self.tempo_cb.currentTextChanged.connect(self._on_tempo)
        r2.addWidget(self.tempo_cb); r2.addStretch()
        root.addLayout(r2)
        root.addWidget(self._sep())

        # ── Sekcja FAL ──
        root.addWidget(self._lbl_sec("🎵 FALA"))
        r3 = QHBoxLayout(); r3.setSpacing(4)
        self._wave_btns = []
        bg = QButtonGroup(self); bg.setExclusive(False)
        self._wave_bg = bg
        for name, wid in self.WAVES:
            b = QPushButton(name); b.setCheckable(True)
            self._wave_btns.append((b, wid)); bg.addButton(b)
            b.clicked.connect(lambda checked, w=wid, btn=b: self._on_wave_btn(w, btn))
            r3.addWidget(b)
        self.btn_remix = QPushButton("🔀 Remix"); self.btn_remix.setCheckable(True)
        self.btn_remix.setStyleSheet(
            "QPushButton:checked{background:#224400;border-color:#88FF00;color:#88FF00}")
        self.btn_remix.toggled.connect(self._on_remix)
        r3.addWidget(self.btn_remix)
        root.addLayout(r3)
        self._wave_lbl = QLabel("● OFF")
        self._wave_lbl.setStyleSheet("color:#00DDFF;font-size:10px;font-weight:bold")
        root.addWidget(self._wave_lbl)
        root.addWidget(self._sep())

        # ── Sekcja CZĘSTOTLIWOŚCI ──
        root.addWidget(self._lbl_sec("📊 CZĘSTOTLIWOŚĆ"))
        frow = QHBoxLayout(); frow.setSpacing(6)
        frow.addWidget(QLabel("Hz:"))
        self.freq_sl = QSlider(Qt.Orientation.Horizontal)
        self.freq_sl.setRange(20,16000); self.freq_sl.setValue(self.FREQ_CENTER)
        self.freq_sl.valueChanged.connect(self._on_freq_sl)
        self.freq_lbl = QLabel(f"{self.FREQ_CENTER} Hz"); self.freq_lbl.setFixedWidth(70)
        self.freq_lbl.setAlignment(Qt.AlignmentFlag.AlignRight)
        frow.addWidget(self.freq_sl,1); frow.addWidget(self.freq_lbl)
        root.addLayout(frow)

        # Skróty Hz
        fq = QHBoxLayout(); fq.setSpacing(3)
        for lbl, hz in [("60",60),("120",120),("250",250),("440",440),
                         ("1k",1000),("2k",2000),("4k",4000),("8k",8000)]:
            b = QPushButton(lbl); b.setFixedWidth(40)
            b.clicked.connect(lambda _, h=hz: self._set_freq(h))
            fq.addWidget(b)
        fq.addStretch(); root.addLayout(fq)
        root.addWidget(self._sep())

        # ── Sekcja AUTO-SWEEP ──
        root.addWidget(self._lbl_sec("🌊 AUTO-SWEEP  (center→low→high→center)"))
        sw_row = QHBoxLayout(); sw_row.setSpacing(6)
        self.btn_sweep = QPushButton("▶ Sweep"); self.btn_sweep.setCheckable(True)
        self.btn_sweep.setStyleSheet(
            "QPushButton:checked{background:#002244;border-color:#0088FF;color:#0088FF}")
        self.btn_sweep.toggled.connect(self._on_sweep)
        sw_row.addWidget(self.btn_sweep)
        for label, attr, val in [("Lo:", "_sw_lo", self.FREQ_LOW),
                                  ("Hi:", "_sw_hi", self.FREQ_HIGH)]:
            sw_row.addWidget(QLabel(label))
            sl = QSlider(Qt.Orientation.Horizontal)
            sl.setRange(20,16000); sl.setValue(val); sl.setFixedWidth(90)
            lbl2 = QLabel(f"{val}Hz"); lbl2.setFixedWidth(52)
            sl.valueChanged.connect(lambda v, l=lbl2: l.setText(f"{v}Hz"))
            sw_row.addWidget(sl); sw_row.addWidget(lbl2)
            setattr(self, attr, sl)
        root.addLayout(sw_row)
        self._sweep_lbl = QLabel("● STOP")
        self._sweep_lbl.setStyleSheet("color:#0066AA;font-size:10px")
        root.addWidget(self._sweep_lbl)
        root.addStretch()

    # ── GStreamer ─────────────────────────────────────────────────────────────

    def set_app(self, app_ref):
        """Referencja do CarbonPhaserPlayer — potrzebna do uruchomienia Monitor Mode."""
        self._app = app_ref

    def _build_pipe(self):
        """
        Pipeline generatora:
          audiotestsrc → volume → audioconvert → audioresample → pulsesink[carbon_monitor]
        Sygnał trafia do virtual sink PulseAudio, skąd Monitor Mode go odbiera
        i puszcza przez cały łańcuch FX (Tape, Spatial, Phantom, EQ, Chain...).
        """
        if self._pipe:
            self._pipe.set_state(Gst.State.NULL)
            self._pipe = None
        p   = Gst.Pipeline.new("tsg-pipe")
        src = mkgst("audiotestsrc","tsg_src",
                    {"wave":0,"freq":float(self.FREQ_CENTER),"volume":1.0,"is-live":True})
        vol  = mkgst("volume","tsg_vol",{"volume":self.vol_sl.value()/100.0})
        conv = mkgst("audioconvert","tsg_conv")
        res  = mkgst("audioresample","tsg_res")
        # Wysyłamy do virtual sink — Monitor Mode go przechwytuje przez .monitor
        snk  = mkgst("pulsesink","tsg_sink",
                     {"device": VIRTUAL_SINK, "sync": False})
        for el in [src,vol,conv,res,snk]:
            if el: p.add(el)
        def lnk(a,b):
            if a and b:
                if not a.link(b): print(f"  [!] tsg: {a.get_name()}->{b.get_name()}")
        lnk(src,vol); lnk(vol,conv); lnk(conv,res); lnk(res,snk)
        self._pipe=p; self._src=src; self._vol_el=vol

    def _s(self, prop, val):
        if self._src:
            try: self._src.set_property(prop, val)
            except Exception as e: print(f"  [TSG] {prop}={val}: {e}")

    # ── Slots ─────────────────────────────────────────────────────────────────

    def _on_start(self, active):
        self._active = active
        if active:
            # 1. Utwórz virtual sink jeśli nie istnieje
            if not create_virtual_sink():
                QMessageBox.critical(self, "Generator",
                    "Nie można utworzyć virtual sink PulseAudio!\n"
                    "Sprawdź czy PulseAudio/PipeWire jest uruchomiony.")
            ...
            # 2. Uruchom generator → pulsesink[carbon_monitor]
            self._build_pipe()
            self._pipe.set_state(Gst.State.PLAYING)
            self.btn_start.setText("⏹ STOP GENERATOR")
            self._set_freq(self.freq_sl.value())
            self._apply_wave_now()

            # 3. Aktywuj Monitor Mode w głównej aplikacji
            #    (pulsesrc[carbon_monitor.monitor] → cały FX chain → output)
            if hasattr(self, '_app') and self._app:
                self._app._start_monitor()
                self._wave_lbl.setText(
                    "✅ Generator → FX Chain aktywny")
            else:
                self._wave_lbl.setText(
                    "⚠ Brak ref do app — Monitor Mode uruchom ręcznie")

            if self.btn_sweep.isChecked(): self._start_sweep()
            if self.btn_remix.isChecked(): self._start_remix()
        else:
            self._sweep_timer.stop(); self._remix_timer.stop()
            if self._pipe:
                self._pipe.set_state(Gst.State.NULL)
                self._pipe = None
            self._src = None; self._vol_el = None
            self.btn_start.setText("▶ START GENERATOR")
            self._wave_lbl.setText("● OFF")
            self._sweep_lbl.setText("● STOP")
            # Informacja — Monitor Mode zostaje aktywny (user może go wyłączyć ręcznie)
            # żeby nie przerywać odsłuchu jeśli user chce coś jeszcze sprawdzić

    def _on_vol(self, v):
        if self._vol_el:
            try: self._vol_el.set_property("volume", v/100.0)
            except: pass

    def _on_wave_btn(self, wave_id, btn):
        # Wyłącz remix
        self.btn_remix.blockSignals(True); self.btn_remix.setChecked(False)
        self.btn_remix.blockSignals(False)
        self._remix_timer.stop()
        # Odznacz resztę
        for b, _ in self._wave_btns:
            if b is not btn: b.setChecked(False)
        if self._active: self._s("wave", wave_id)
        name = next((n for n,w in self.WAVES if w==wave_id), "?")
        self._wave_lbl.setText(f"● {name}  ({int(self._sweep_freq)} Hz)")

    def _apply_wave_now(self):
        for b, wid in self._wave_btns:
            if b.isChecked(): self._s("wave", wid); return
        self._s("wave", 0)

    def _on_tempo(self, t):
        if self._sweep_timer.isActive(): self._sweep_timer.setInterval(self.TEMPO_MS[t])
        if self._remix_timer.isActive(): self._remix_timer.setInterval(self.WAVE_HOLD[t])

    # ── Freq ─────────────────────────────────────────────────────────────────

    def _set_freq(self, hz):
        self._sweep_freq = float(hz)
        self.freq_sl.blockSignals(True); self.freq_sl.setValue(int(hz))
        self.freq_sl.blockSignals(False)
        self.freq_lbl.setText(f"{int(hz)} Hz")
        if self._active: self._s("freq", float(hz))

    def _on_freq_sl(self, v):
        # Ręczny suwak — stop sweep
        if self.btn_sweep.isChecked():
            self.btn_sweep.blockSignals(True); self.btn_sweep.setChecked(False)
            self.btn_sweep.blockSignals(False)
            self._sweep_timer.stop(); self._sweep_lbl.setText("● STOP (ręczny)")
        self._set_freq(v)

    # ── Sweep ─────────────────────────────────────────────────────────────────

    def _on_sweep(self, active):
        if active: self._start_sweep()
        else:
            self._sweep_timer.stop(); self._sweep_lbl.setText("● STOP")

    def _start_sweep(self):
        t = self.tempo_cb.currentText()
        self._sweep_freq  = float(self.FREQ_CENTER)
        self._sweep_phase = 0   # 0: center→low  1: low→high  2: high→center
        self._sweep_timer.setInterval(self.TEMPO_MS[t])
        self._sweep_timer.start()

    def _sweep_tick(self):
        t    = self.tempo_cb.currentText()
        step = self.TEMPO_STEP[t]
        lo   = float(max(20,   self._sw_lo.value()))
        hi   = float(min(16000,self._sw_hi.value()))

        if self._sweep_phase == 0:          # center → low
            self._sweep_freq /= step
            if self._sweep_freq <= lo:
                self._sweep_freq = lo; self._sweep_phase = 1
                self._sweep_lbl.setStyleSheet("color:#00FF88;font-size:10px")
            else:
                self._sweep_lbl.setText(f"↓ center→low   {int(self._sweep_freq)} Hz")
                self._sweep_lbl.setStyleSheet("color:#FF8800;font-size:10px")

        elif self._sweep_phase == 1:        # low → high
            self._sweep_freq *= step
            if self._sweep_freq >= hi:
                self._sweep_freq = hi; self._sweep_phase = 2
                self._sweep_lbl.setStyleSheet("color:#FF4444;font-size:10px")
            else:
                self._sweep_lbl.setText(f"↑ low→high   {int(self._sweep_freq)} Hz")
                self._sweep_lbl.setStyleSheet("color:#00FF88;font-size:10px")

        else:                               # high → center
            self._sweep_freq /= step
            if self._sweep_freq <= float(self.FREQ_CENTER):
                self._sweep_freq = float(self.FREQ_CENTER); self._sweep_phase = 0
            self._sweep_lbl.setText(f"↓ high→center  {int(self._sweep_freq)} Hz")
            self._sweep_lbl.setStyleSheet("color:#FF8800;font-size:10px")

        self.freq_sl.blockSignals(True); self.freq_sl.setValue(int(self._sweep_freq))
        self.freq_sl.blockSignals(False)
        self.freq_lbl.setText(f"{int(self._sweep_freq)} Hz")
        if self._active: self._s("freq", self._sweep_freq)

    # ── Remix fal ─────────────────────────────────────────────────────────────

    def _on_remix(self, active):
        if active: self._start_remix()
        else:
            self._remix_timer.stop()
            for b, _ in self._wave_btns: b.blockSignals(True); b.setChecked(False); b.blockSignals(False)

    def _start_remix(self):
        t = self.tempo_cb.currentText()
        self._remix_idx = 0
        self._remix_timer.setInterval(self.WAVE_HOLD[t])
        self._remix_timer.start()
        self._remix_tick()

    def _remix_tick(self):
        t = self.tempo_cb.currentText()
        self._remix_timer.setInterval(self.WAVE_HOLD[t])
        si   = self.REMIX_SEQ[self._remix_idx % len(self.REMIX_SEQ)]
        name, wid = self.WAVES[si]
        if self._active: self._s("wave", wid)
        for i, (b, _) in enumerate(self._wave_btns):
            b.blockSignals(True); b.setChecked(i==si); b.blockSignals(False)
        self._wave_lbl.setText(f"🔀 {name}  ({int(self._sweep_freq)} Hz)")
        self._remix_idx += 1
...
class CarbonPhaserPlayer(QMainWindow):
    ...
    def _gst_init(self):
        ...
        # Phantom Stereo — własny niezależny blok za Spatial FX
        # Topologia: sp_echo → ph_conv1 → ph_stereo → ph_conv2 → ph_echo → DSPResolver
        # Bypass: ph_stereo.stereo=1.0, ph_echo.intensity=0.0 (sygnał bez zmian)
        self.ph_conv1  = mkgst("audioconvert","ph_conv1")
        self.ph_stereo = mkgst("stereo",      "ph_stereo", {"stereo":1.0})
        self.ph_conv2  = mkgst("audioconvert","ph_conv2")
        self.ph_echo   = mkgst("audioecho",   "ph_echo",
                               {"delay":1,"intensity":0.0,"feedback":0.0})
        ...
        # Domyślne połączenie: ph_echo → conv_out (brak aktywnych modułów chain)
        lnk(self.ph_echo, self.conv_out)
    ...
    def _build_ui(self):
        ...
        # Tab 5: Test Signal Generator
        t5 = QWidget(); t5v = QVBoxLayout(t5)
        t5v.setContentsMargins(4,4,4,4); t5v.setSpacing(4)
        self.test_gen = TestSignalGenerator()
        t5v.addWidget(self.test_gen); t5v.addStretch()
        self.tabs.addTab(t5, "🔬 Generator")
    ...
    def _connect_widgets(self):
        ...
        # Phantom Stereo — własne elementy GST (ph_*), niezależne od Spatial
        # Elementy są już wstawione w pipeline statycznie; widget tylko nimi steruje
        self.phantom_stereo.ph_conv1  = self.ph_conv1
        self.phantom_stereo.ph_stereo = self.ph_stereo
        self.phantom_stereo.ph_conv2  = self.ph_conv2
        self.phantom_stereo.ph_echo   = self.ph_echo
        self.phantom_stereo._apply_current()   # zastosuj domyślny stan (bypass)
        ...
        # Generator testowy — referencja do app dla Monitor Mode
        self.test_gen.set_app(self)
    ...
    def _stop_monitor_pipe(self):
        ...
        # Wyczyść mph_* z phantom — pipeline monitora już nie istnieje
        self.phantom_stereo.mph_conv1  = None
        self.phantom_stereo.mph_stereo = None
        self.phantom_stereo.mph_conv2  = None
        self.phantom_stereo.mph_echo   = None
    ...
    def _start_mon_pipe(self, device, name):
        ...
        # Phantom monitor — własny niezależny blok za Spatial monitor
        mph_conv1  = mkgst("audioconvert","mon_ph_conv1")
        mph_stereo = mkgst("stereo",      "mon_ph_stereo", {"stereo":1.0})
        mph_conv2  = mkgst("audioconvert","mon_ph_conv2")
        mph_echo   = mkgst("audioecho",   "mon_ph_echo",
                           {"delay":1,"intensity":0.0,"feedback":0.0})
        ...
        for el in ([src,conv,res,tee,q_fx,t_sat,t_gn,t_tn,eq10,sp_sat,
                    ...
                    mph_conv1,mph_stereo,mph_conv2,mph_echo]
        ...
        # Domyślne połączenie — jeśli brak aktywnych, mph_echo → cvo bezpośrednio
        ...
        # Phantom monitor — własne mph_* elementy, niezależne od Spatial
        self.phantom_stereo.mph_conv1  = mph_conv1
        self.phantom_stereo.mph_stereo = mph_stereo
        self.phantom_stereo.mph_conv2  = mph_conv2
        self.phantom_stereo.mph_echo   = mph_echo
        self.phantom_stereo._apply_current()   # sync stanu (bypass lub aktywny)

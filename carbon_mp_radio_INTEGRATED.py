...
# ============================================================================
# TAPE SPATIAL PROCESSOR - Core Audio Engine
# ============================================================================
class TapeSpatialProcessor:
    """Professional spatial audio processing engine"""
    
    def __init__(self, sample_rate=44100):
        self.sample_rate = sample_rate
        self.phase_shift_left = 0.0
        self.phase_shift_right = 0.0
        self.haas_delay_ms = 0.0
        self.haas_channel = 'right'
        ...
        self.tape_saturation = 0.0
        self.wow_flutter_amount = 0.0
    ...
    def apply_phase_shift(self, audio, phase_degrees):
        if phase_degrees == 0:
            return audio
        audio_fft = np.fft.fft(audio)
        phase_rad = np.deg2rad(phase_degrees)
        phase_shift = np.exp(1j * phase_rad)
        audio_fft *= phase_shift
        return np.real(np.fft.ifft(audio_fft))
    
    def apply_haas_effect(self, left, right):
        if self.haas_delay_ms == 0:
            return left, right
        delay_samples = int((self.haas_delay_ms / 1000.0) * self.sample_rate)
        if self.haas_channel == 'right':
            delayed = np.pad(right, (delay_samples, 0), mode='constant')[:len(right)]
            return left, delayed
        else:
            delayed = np.pad(left, (delay_samples, 0), mode='constant')[:len(left)]
            return delayed, right
    
    def apply_mid_side_widening(self, left, right, width):
        if width == 0:
            mono = (left + right) / 2
            return mono, mono
        mid = (left + right) / 2
    ...
    def split_frequency_bands(self, audio):
        b_low, a_low = self.butter_lowpass(250)
        b_high, a_high = self.butter_highpass(4000)
        b_mid, a_mid = self.butter_bandpass(250, 4000)
        
        low = filtfilt(b_low, a_low, audio)
        mid = filtfilt(b_mid, a_mid, audio)
        high = filtfilt(b_high, a_high, audio)
        
        return low, mid, high
    
    def apply_eq_boost(self, audio, boost_db):
        if boost_db == 0:
            return audio
        linear_gain = 10 ** (boost_db / 20.0)
        return audio * linear_gain
    
    def apply_tape_saturation(self, audio, amount):
        if amount == 0:
            return audio
        drive = 1 + (amount / 100.0) * 3
        driven = audio * drive
        saturated = np.tanh(driven)
        return saturated / drive * 1.5
    
    def apply_wow_flutter(self, audio, amount):
        if amount == 0:
            return audio
        length = len(audio)
        wow_freq = 0.8
        self.wow_phase += (wow_freq * length) / self.sample_rate
        wow_mod = np.sin(2 * np.pi * wow_freq * np.arange(length) / self.sample_rate + self.wow_phase)
        flutter_freq = 7.5
        self.flutter_phase += (flutter_freq * length) / self.sample_rate
        flutter_mod = np.sin(2 * np.pi * flutter_freq * np.arange(length) / self.sample_rate + self.flutter_phase)
        pitch_mod = (wow_mod * 0.002 + flutter_mod * 0.001) * (amount / 100.0)
        indices = np.arange(length) * (1 + pitch_mod)
        indices = np.clip(indices, 0, length - 1).astype(int)
        return audio[indices]
    
    def process_stereo(self, left, right):
        """Main processing pipeline"""
        left_low, left_mid, left_high = self.split_frequency_bands(left)
        right_low, right_mid, right_high = self.split_frequency_bands(right)
        
        left_low = self.apply_eq_boost(left_low, self.low_boost)
        right_low = self.apply_eq_boost(right_low, self.low_boost)
        left_mid = self.apply_eq_boost(left_mid, self.mid_boost)
        right_mid = self.apply_eq_boost(right_mid, self.mid_boost)
        left_high = self.apply_eq_boost(left_high, self.high_boost)
        right_high = self.apply_eq_boost(right_high, self.high_boost)
        
        left_low, right_low = self.apply_mid_side_widening(left_low, right_low, self.low_width)
        left_mid, right_mid = self.apply_mid_side_widening(left_mid, right_mid, self.mid_width)
        left_high, right_high = self.apply_mid_side_widening(left_high, right_high, self.high_width)
        
        left_combined = left_low + left_mid + left_high
        right_combined = right_low + right_mid + right_high
        
        if self.phase_shift_left != 0:
            left_combined = self.apply_phase_shift(left_combined, self.phase_shift_left)
        if self.phase_shift_right != 0:
            right_combined = self.apply_phase_shift(right_combined, self.phase_shift_right)
        
        left_combined, right_combined = self.apply_haas_effect(left_combined, right_combined)
        
        if self.tape_saturation > 0:
            left_combined = self.apply_tape_saturation(left_combined, self.tape_saturation)
            right_combined = self.apply_tape_saturation(right_combined, self.tape_saturation)
        
        if self.wow_flutter_amount > 0:
            left_combined = self.apply_wow_flutter(left_combined, self.wow_flutter_amount)
            right_combined = self.apply_wow_flutter(right_combined, self.wow_flutter_amount)
        
        max_val = max(np.abs(left_combined).max(), np.abs(right_combined).max())
        if max_val > 0.95:
            left_combined = left_combined / max_val * 0.95
            right_combined = right_combined / max_val * 0.95
...
# ============================================================================
# TAPE SPATIAL WIDGET - Compact UI Control
# ============================================================================
class TapeSpatialWidget(QGroupBox):
    """Compact Tape Spatial FX control widget"""
    
    def __init__(self, parent=None):
        super().__init__("🎚️ TAPE SPATIAL FX", parent)
        self.processor = TapeSpatialProcessor()
    ...
    def init_ui(self):
        ...
        self.preset_combo.addItems(["Flat", "Studio", "Vintage", "Psychedelic", "Haas", "Phase"])
        ...
        # Phase
        phase_h = QHBoxLayout()
        phase_h.addWidget(QLabel("Phase L/R:"))
        self.phase_left_slider = self.create_compact_slider(0, 180, 0)
        self.phase_left_slider.valueChanged.connect(self.on_phase_left_changed)
        phase_h.addWidget(self.phase_left_slider)
        self.phase_right_slider = self.create_compact_slider(0, 180, 0)
        self.phase_right_slider.valueChanged.connect(self.on_phase_right_changed)
        phase_h.addWidget(self.phase_right_slider)
        grid_layout.addLayout(phase_h)
        
        # Haas
        haas_h = QHBoxLayout()
        haas_h.addWidget(QLabel("Haas Delay:"))
        self.haas_delay_slider = self.create_compact_slider(0, 40, 0)
        self.haas_delay_slider.valueChanged.connect(self.on_haas_delay_changed)
        haas_h.addWidget(self.haas_delay_slider)
        self.haas_label = QLabel("0ms")
        haas_h.addWidget(self.haas_label)
        grid_layout.addLayout(haas_h)
        
        # Width
        width_h = QHBoxLayout()
        width_h.addWidget(QLabel("Width L/M/H:"))
        self.low_width_slider = self.create_compact_slider(0, 100, 0)
        self.low_width_slider.valueChanged.connect(self.on_low_width_changed)
        width_h.addWidget(self.low_width_slider)
        self.mid_width_slider = self.create_compact_slider(0, 100, 50)
        self.mid_width_slider.valueChanged.connect(self.on_mid_width_changed)
        width_h.addWidget(self.mid_width_slider)
        self.high_width_slider = self.create_compact_slider(0, 100, 80)
        self.high_width_slider.valueChanged.connect(self.on_high_width_changed)
        width_h.addWidget(self.high_width_slider)
        grid_layout.addLayout(width_h)
        
        # Saturation
        sat_h = QHBoxLayout()
        sat_h.addWidget(QLabel("Saturation:"))
        self.saturation_slider = self.create_compact_slider(0, 100, 0)
        self.saturation_slider.valueChanged.connect(self.on_saturation_changed)
        sat_h.addWidget(self.saturation_slider)
        self.sat_label = QLabel("0%")
        sat_h.addWidget(self.sat_label)
        grid_layout.addLayout(sat_h)
    ...
    def on_phase_left_changed(self, value):
        self.processor.phase_shift_left = float(value)
    
    def on_phase_right_changed(self, value):
        self.processor.phase_shift_right = float(value)
    
    def on_haas_delay_changed(self, value):
        self.processor.haas_delay_ms = float(value)
        self.haas_label.setText(f"{value}ms")
    
    def on_low_width_changed(self, value):
        self.processor.low_width = value / 100.0
    
    def on_mid_width_changed(self, value):
        self.processor.mid_width = value / 100.0
    
    def on_high_width_changed(self, value):
        self.processor.high_width = value / 100.0
    
    def on_saturation_changed(self, value):
        self.processor.tape_saturation = float(value)
        self.sat_label.setText(f"{value}%")
    
    def load_preset(self, preset_name):
        presets = {
            "Flat": {'phase_left': 0, 'phase_right': 0, 'haas': 0, 'low_width': 0, 'mid_width': 50, 'high_width': 80, 'saturation': 0},
            "Studio": {'phase_left': 0, 'phase_right': 0, 'haas': 15, 'low_width': 20, 'mid_width': 60, 'high_width': 85, 'saturation': 0},
            "Vintage": {'phase_left': 0, 'phase_right': 0, 'haas': 8, 'low_width': 10, 'mid_width': 40, 'high_width': 60, 'saturation': 35},
            "Psychedelic": {'phase_left': 45, 'phase_right': 90, 'haas': 25, 'low_width': 40, 'mid_width': 80, 'high_width': 100, 'saturation': 20},
            "Haas": {'phase_left': 0, 'phase_right': 0, 'haas': 30, 'low_width': 10, 'mid_width': 70, 'high_width': 90, 'saturation': 5},
            "Phase": {'phase_left': 60, 'phase_right': 120, 'haas': 20, 'low_width': 50, 'mid_width': 90, 'high_width': 95, 'saturation': 15}
        }
        
        if preset_name in presets:
            p = presets[preset_name]
            self.phase_left_slider.setValue(p['phase_left'])
            self.phase_right_slider.setValue(p['phase_right'])
            self.haas_delay_slider.setValue(p['haas'])
            self.low_width_slider.setValue(p['low_width'])
            self.mid_width_slider.setValue(p['mid_width'])
            self.high_width_slider.setValue(p['high_width'])
            self.saturation_slider.setValue(p['saturation'])

...
class CarbonPhaserPlayer(QMainWindow):
    ...
    def gst_init(self):
        ...
        if self.eq:
            # Simple chain: Conv -> TapeSat -> TapeGain -> TapeTone -> EQ -> Spec -> Sink
    ...
    def pl_t(self,i): 
        ...
        if uri.startswith("pulsesrc://"):
            ...
            # Create monitoring pipeline if needed
            if not hasattr(self, '_monitor_pipeline') or not self._monitor_pipeline:
                print(f"🎤 Creating monitoring pipeline for: {device}")
                ...
                self._monitor_pipeline.add(self.pipeline_bin)
                
                # Link
                if not monitor_src.link(self.pipeline_bin):
                    print("❌ Failed to link monitor pipeline")
                    return
                
                # Connect to existing bus for spectrum messages
